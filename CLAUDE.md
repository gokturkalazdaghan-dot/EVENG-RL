...
6. **18+ checkbox only** (`legal-gate.tsx`). No year spinner. The gate also asks for a **date of birth, optional**. The user never picks or types a star sign; `src/lib/natal.ts` derives a temperament from the date and the readings weave it in. Never surface the sign, the system it comes from, or any of its vocabulary. `natal.test.ts` scans the rendered readings for leaks.
