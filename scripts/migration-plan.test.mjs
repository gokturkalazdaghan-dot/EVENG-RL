...
test("the auth schema ships outside the globbed directory", () => {
  // ÖLÇÜLEN DEĞİŞMEZ: globlanan dizinde uygulanacak migration OLMAMASI.
  // Bu üründe oturum açma kapalı (CLAUDE.md: hesap yok, kalıcılık
  // localStorage) ve `migrations/` hiç yok — bu da "bekleyen migration yok"
  // demektir, eksiklik değil. Dizin yoksa `readdirSync` ENOENT atıyordu:
  // test, ölçmek istediği şeyi değil dizinin varlığını ölçüyordu.
  //
  // `migrations/auth/` VARSA eski iddia aynen sürüyor: park edilmiş
  // Better Auth şeması orada durmalı ve globlanan dizine sızmamalı.
  const migrationsDir = join(projectRoot(), "migrations");
  const entries = existsSync(migrationsDir) ? readdirSync(migrationsDir) : [];
  assert.deepEqual(pendingMigrations(entries, []), []);

  const authDir = join(migrationsDir, "auth");
  if (existsSync(authDir)) {
    assert.ok(readdirSync(authDir).includes(AUTH_MIGRATION));
  }
});
