...
test("every hand-over the og skill prints is one this script accepts", (t) => {
  ...
  //
  // Bu belgeler PLATFORMUN ajan iskelesine ait, EVENGIRL'e değil, ve bu
  // deponun devrinde yoklar. Yokken ENOENT ile kırmızıya dönüyordu: test,
  // "belgedeki komutlar bu CLI'ın kabul ettiği komutlar mı" sorusunu değil
  // dizinin varlığını ölçüyordu. Belgeler geri konursa kendiliğinden koşar.
  const skillDir = join(TEMPLATE_ROOT, ".grok/skills/og");
  if (!existsSync(join(skillDir, "SKILL.md")) || !existsSync(join(skillDir, "references"))) {
    return t.skip("bu depoda .grok/skills/og belgeleri yok");
  }
  const docs = [
