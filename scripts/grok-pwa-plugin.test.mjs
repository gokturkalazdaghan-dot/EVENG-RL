...
/**
 * BOŞ bir çalışma dizini.
 *
 * `injectGrokPwaHead`, `ctx.site` verilmediğinde `cwd`'deki
 * `src/lib/og/site.json` dosyasını OKUR — üretimde doğru davranış.
 * Ama bu testlerin çoğu `site` vermeden çağırıyor ve o zaman deponun
 * KENDİ site.json'ı teste sızıyor: "belge başlığı og:title olur"
 * iddiası, depoda site.json yokken doğru görünüyor, eklendiği anda
 * kırılıyor. Test, ölçtüğünü sandığı şeyi ölçmüyordu.
 *
 * Ölçüldü: bu dizin verilmeden altı test, yalnızca `src/lib/og/site.json`
 * var diye kırmızıya döndü — enjektörde hiçbir şey değişmeden.
 */
const BARE_CWD = mkdtempSync(join(tmpdir(), "grok-bare-cwd-"));
