...
/**
 * Bu üç test, PLATFORMUN ajan belgelerini (`.grok/skills/og/SKILL.md`,
 * `AGENTS.md`) betiklerin davranışına sabitler. O belgeler EVENGIRL'e ait
 * değil — app-builder iskelesinin parçası ve bu deponun devrinde yoklar.
 *
 * Yokken ENOENT ile kırmızıya dönüyorlardı: test, ölçmek istediği
 * "belge ile kod aynı şeyi söylüyor mu" sorusunu değil, belgenin varlığını
 * ölçüyordu. Belgeler geri konursa üçü de kendiliğinden yeniden koşar.
 *
 * SESSİZ DELİK OLMASIN DİYE: aşağıdaki "belge kümesi ya tam ya hiç" testi,
 * yarım kalmış bir geri koymayı yakalar — biri varken diğeri yoksa
 * kırmızıya döner, atlanmaz.
 */
const OG_SKILL_DOCS = [".grok/skills/og/SKILL.md", "AGENTS.md"];
const ogDocsPresent = OG_SKILL_DOCS.every((rel) => existsSync(join(TEMPLATE_ROOT, rel)));

test("the og skill doc set is all-or-nothing", () => {
  const present = OG_SKILL_DOCS.filter((rel) => existsSync(join(TEMPLATE_ROOT, rel)));
  assert.ok(
    present.length === 0 || present.length === OG_SKILL_DOCS.length,
    `yarım belge kümesi: ${present.join(", ")} var, geri kalanı yok`,
  );
});

test("SKILL.md and AGENTS.md name the marker path and bound this script uses", (t) => {
  if (!ogDocsPresent) return t.skip("bu depoda .grok/skills/og belgeleri yok — bkz. yukarıdaki not");
